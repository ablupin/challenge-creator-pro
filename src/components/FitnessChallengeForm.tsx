...
import { ArrowLeft, ArrowRight, Upload, X, Dumbbell } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Textarea } from '@/components/ui/textarea';
import { FitnessChallengeInput } from '@/types/challenge';

interface FitnessChallengeFormProps {
  onSubmit: (input: FitnessChallengeInput) => void;
  onBack: () => void;
}
...
export function FitnessChallengeForm({ onSubmit, onBack }: FitnessChallengeFormProps) {
  ...
  const isValid = workoutTheme.trim().length > 0;
    ...
    >
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="font-display text-2xl md:text-3xl font-bold">Fitness Challenge</h2>
            <p className="text-muted-foreground">Set up your workout program</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="exercises" className="text-base font-semibold">Exercises Per Workout</Label>
            <Input
              id="exercises"
              type="number"
              min={1}
        ...
        <div className="space-y-2">
          <Label htmlFor="theme" className="text-base font-semibold">Workout Theme</Label>
          <Textarea
            id="theme"
            placeholder="e.g., HIIT fat burning, Glute focus strength training, Full body beginner, Upper body muscle building..."
            value={workoutTheme}
            onChange={(e) => setWorkoutTheme(e.target.value)}
            className="min-h-[100px] text-base resize-none"
          />
          <p className="text-sm text-muted-foreground">Describe the workout style or goal</p>
        </div>
        ...
        <div className="flex items-center justify-between pt-6">
          ...
          <Button
            type="submit"
            disabled={!isValid}
            className="gap-2 bg-gradient-fitness hover:opacity-90 text-white"
          >
            Generate with AI
